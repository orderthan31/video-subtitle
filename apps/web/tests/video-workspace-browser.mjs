...
try {
  ...
  const asset = {
    ...
    media: { duration: 5, width: 640, height: 360, has_audio: true },
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (path === '/api/videos') {
      return route.fulfill({ json: { videos: [asset] } });
    }
    if (path === '/api/jobs') {
    ...
    if (path === '/api/video-uploads') {
      if (method === 'POST') {
        const data = body();
        assert.deepEqual(Object.keys(data).sort(), ['filename', 'request_id', 'size']);
        const upload = {
          upload_id: String(uploads.length + 1).repeat(32),
    ...
    if (path.startsWith('/api/video-uploads/')) {
      const id = path.split('/')[3],
        upload = uploads.find((u) => u.upload_id === id);
      assert.ok(upload);
      if (path.endsWith('/chunks')) {
        chunkActive++;
        maxChunkActive = Math.max(maxChunkActive, chunkActive);
        await new Promise((r) => setTimeout(r, 600));
        upload.uploaded_bytes += route.request().postDataBuffer().length;
        chunkActive--;
        return route.fulfill({ json: upload });
      }
      if (path.endsWith('/complete')) {
        upload.status = 'READY';
        upload.asset_id = asset.asset_id;
        return route.fulfill({ json: { status: 'ready', video: asset } });
      }
      return route.fulfill({ json: { ...upload, resumable: upload.status === 'UPLOADING' } });
    }
    if (path.endsWith('/workflow-plan')) {
      const b = body();
      let stages = ['analyze'];
      if (['full', 'transcribe', 'transcribe_translate'].includes(b.template)) {
        stages.push('extract_audio', 'preprocess_audio', 'transcribe');
      }
      if (['full', 'translate', 'transcribe_translate'].includes(b.template)) {
        stages.push('translate');
      }
      if (b.template !== 'extract_audio') {
        stages.push('generate_subtitle');
      }
      if (['full', 'encode'].includes(b.template)) {
        stages.push('encode', 'validate_video');
      }
      return route.fulfill({
        json: {
          stages,
          reused: [],
          paid_stages: stages.filter((s) => ['transcribe', 'translate'].includes(s)),
        },
      });
    }
    if (path.endsWith('/audio-inputs')) {
      return route.fulfill({ json: { audio_inputs: [] } });
    }
    if (path === `/api/videos/${asset.asset_id}/subtitles`) {
      return route.fulfill({ json: { subtitles: [subtitle] } });
    }
    if (path === `/api/videos/${asset.asset_id}/jobs`) {
      created.push(body());
      const job = {
        job_id: 'b'.repeat(32),
        status: 'COMPLETED',
        ...
        expected_size: media.length,
        uploaded_bytes: media.length,
        created_at: asset.created_at,
        error: null,
        metadata: {
          asset_id: asset.asset_id,
          workflow: { template: body().template },
          result_files: ['original.srt'],
          duration: 5,
        },
      };
      jobs = [job];
    ...
    if (path === `/api/videos/${asset.asset_id}`) {
      return route.fulfill({ json: { video: asset, jobs } });
    }
        ...
        json: {
          ...
          tracks: {
            original: {
              available: true,
              partial: false,
              cues: [{ start: 1, end: 2, text: 'Original dialogue' }],
            },
            translated: { available: false, partial: false, cues: [] },
          },
        },
      });
    }
    if (path === `/api/jobs/${'b'.repeat(32)}`) {
  ...
  await page.waitForFunction(
    () => document.querySelector('.asset-overview video')?.readyState >= 1,
  );
  ...
  for (const width of [320, 390, 768, 1440]) {
    ...
    assert.ok(
      await page.locator('.workflow-modal').evaluate((el) => el.scrollWidth <= el.clientWidth + 1),
      `modal overflow at ${width}`,
    );
    if (width === 390) {
      await page.screenshot({ path: 'data/mvp2-workflow-mobile.png', fullPage: true });
    }
  ...
  await page
    ...
    .first()
    .setInputFiles([
      { name: '여행 영상 (최종).mp4', mimeType: 'video/mp4', buffer: Buffer.alloc(100) },
      { name: '1001.mp4', mimeType: 'video/mp4', buffer: Buffer.alloc(100) },
    ]);
  await page.waitForFunction(() =>
    document.querySelector('.upload-shelf')?.textContent.includes('여행 영상 (최종).mp4'),
  );
  await page.locator('.asset-row').click();
  await page.waitForTimeout(1800);
  assert.equal(uploads.length, 2);
  assert.ok(uploads.every((u) => u.asset_id));
  assert.equal(maxChunkActive, 1);
  assert.deepEqual(
    uploads.map((u) => u.filename),
    ['여행 영상 (최종).mp4', '1001.mp4'],
  );
  assert.equal(await page.locator('input[type=file]').first().inputValue(), '');
  assert.deepEqual(errors, []);
  console.log(
    'MVP2 browser passed: library/detail, workflow creation, original playback, 4 widths, serial uploads across navigation.',
  );
} finally {
  await browser.close();
}
