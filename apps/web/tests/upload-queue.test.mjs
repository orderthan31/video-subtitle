...
test('uploads run serially with independent settings and release completed files', async () => {
  ...
  assert.deepEqual(started, [['one.mp4','first'],['two.mp4','second']]);
...
test('failed upload does not block the queue; retry keeps its identity and server job', async () => {
  ...
  const queue = new UploadQueue(async item => {
    ...
    if (item.name === 'one.mp4' && fail) throw new Error('offline');
  });
  queue.add(file('one.mp4'), {}); queue.add(file('two.mp4'), {});
  await flush();
  const key = queue.items[0].key;
  assert.equal(queue.items[0].state, 'error');
  assert.equal(queue.items[1].state, 'done');
  fail = false; queue.resume(key); await flush();
...
test('hidden page stops admissions, then resumes without parallel transfers', async () => {
  ...
  const queue = new UploadQueue(async (_item, signal) => {
    count++; active++; peak = Math.max(active,peak);
    await new Promise(resolve => signal.addEventListener('abort', resolve, {once:true}));
    active--;
  });
  queue.add(file('one.mp4'), {}); queue.add(file('two.mp4'), {});
  queue.suspend(true); await flush();
  assert.equal(count, 1); assert.equal(queue.items[0].state, 'waiting');
  queue.suspend(false); await flush();
  assert.equal(count, 2); assert.equal(peak, 1);
  queue.pause(queue.items[0].key); await flush();
  assert.equal(queue.items[0].state, 'paused'); assert.equal(count, 3);
  queue.dispose(); await flush();
});
...
test('strict-mode setup cleanup setup does not disable queue and resume avoids duplicates', async () => {
  ...
  queue.dispose(); queue.activate(); queue.suspend(true);
  queue.add(file('one.mp4'), {}, 'existing'); queue.add(file('one.mp4'), {}, 'existing');
