...
test('video selection accepts real filenames and video MIME types without renaming', async()=>{
  const seen=[];
  const queue=new UploadQueue(async item=>{seen.push(item.name);});
...
test('generic file picker rejects non-video selections',()=>{
  for(const file of [{name:'photo.jpg',type:'image/jpeg'}, {name:'subtitle.srt',type:''},
    {name:'script.mp4.exe',type:'application/octet-stream'}, {name:'notes.txt',type:'text/plain'}]){
