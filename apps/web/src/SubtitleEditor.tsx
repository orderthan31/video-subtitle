...
const pageSize = 30;
export function SubtitleEditor({job,onClose,onRendered}:{job:Job;onClose:()=>void;onRendered:()=>void}) {
  const [draft,setDraft] = useState<Draft|null>(null), [track,setTrack] = useState('translated');
  ...
  function update(index:number, changes:Partial<Cue>) {
    ...
    setDirty(true);setDiscard(false);
  }
  function remove(index:number) {
    if(!draft||draft.tracks[track].length<=1)return;
    const next=draft.tracks[track].filter((_,i)=>i!==index);
    setDraft({...draft,tracks:{...draft.tracks,[track]:next}});
    setPage(Math.min(page,Math.floor((next.length-1)/pageSize)));setDirty(true);
  }
  function add() {
    if(!draft)return;
  ...
  return <div className="overlay"><section role="dialog" aria-modal="true" aria-labelledby="subtitle-editor-title" className="subtitle-editor" onKeyDown={event=>{
    ...
    {!draft?(!error&&<p>자막 불러오는 중</p>):<>
      ...
      <div className="editor-cues"><div className="cue-heading"><span>시작 (초)</span><span>종료 (초)</span><span>문구</span><span/></div>{cues.slice(page*pageSize,(page+1)*pageSize).map((cue,offset)=>{const index=page*pageSize+offset;return <div className="cue-row" key={`${track}-${index}`}>
        ...
        <button className="icon danger" title={`자막 ${index+1} 삭제`} disabled={saving||cues.length<=1} onClick={()=>remove(index)}><Trash2 size={16}/></button>
      </div>;})}</div>
      <div className="editor-footer"><div><button className="icon" title="이전 자막 페이지" disabled={page===0} onClick={()=>setPage(page-1)}><ChevronLeft size={18}/></button><span>{page+1} / {pages}</span><button className="icon" title="다음 자막 페이지" disabled={page+1>=pages} onClick={()=>setPage(page+1)}><ChevronRight size={18}/></button></div><div><button className="icon" title="초안 저장" disabled={saving||!dirty} onClick={()=>void save('save')}><Save size={19}/></button><button disabled={saving} onClick={()=>void save('render')}><Play size={16}/>최종 출력</button></div></div>
    </>}
