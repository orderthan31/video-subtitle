...
export function StageProgress({job}: {job: Job}) {
  ...
  if (translation && (job.status === 'TRANSLATING' || (['FAILED', 'CANCELLED'].includes(job.status) && job.metadata.failed_stage === 'TRANSLATING'))) {
    ...
    return <div className="stage-progress" aria-live="polite">
      <p>번역 ({translation.language}) {completed} / {translation.total} 묶음 · {percent}%
        {job.status === 'TRANSLATING' && <> · 요청 중 {translation.in_flight}{translation.retrying > 0 && ` · 재시도 대기 ${translation.retrying}`}{translation.draining && ' · 잔여 요청 회수 중'}</>}
        {translation.failed > 0 && ` · 실패 ${translation.failed}`}</p>
      <progress aria-label="번역 진행률" aria-valuetext={`${completed} / ${translation.total} 묶음 완료, ${percent}%`} max={Math.max(1, translation.total)} value={translation.total ? completed : 1}/>
    </div>;
  }
