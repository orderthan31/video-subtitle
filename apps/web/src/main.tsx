...
function App() {
  ...
  return <>
    ...
    <main>
      <div className="page-title"><div><p className="eyebrow">WORKSPACE</p><h1>영상 번역</h1></div><span>{jobs.filter(job=>!terminal(job)).length+localUploads.filter(item=>!jobs.some(job=>job.job_id===item.jobId)).length}개 진행 중</span></div>
      ...
      <section className="jobs" aria-label="작업 목록"><div className="section-title"><h2>작업 목록 <span>{jobs.length+localUploads.filter(item=>!jobs.some(job=>job.job_id===item.jobId)).length}</span></h2><button className="icon" title="목록 새로고침" onClick={()=>void refresh()}><RefreshCw size={18}/></button></div>
        ...
        </article>:job?<article key={entry.key} className="job-row"><div className="video-icon">{job.status==='COMPLETED'?<Check/>:<FileVideo/>}</div><div className="job-description"><h3>{job.original_filename}</h3><p>{bytes(job.expected_size)} <span>·</span> {languageName(job.target_language)} <span>·</span> {new Date(job.created_at).toLocaleString('ko-KR')}</p>{job.error&&<p className="job-error">{job.error}</p>}{job.status==='UPLOADING'&&<progress aria-label={`${job.original_filename} 업로드 진행률`} max={job.expected_size} value={job.uploaded_bytes}/>}</div><span className={'status '+job.status.toLowerCase()}><Clock size={14}/>{job.metadata.cancel_requested&&!terminal(job)?'취소 중':labels[job.status]||job.status}</span><div className="actions">
