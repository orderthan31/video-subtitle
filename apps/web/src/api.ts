...
export type Job = {
  ...
  metadata: {
    ...
    failure?: {
      ...
      provider: string;
      stage: string;
      blocks: { segment?: number; reason: string }[];
    } | null;
    asset_id?: string;
    workflow?: {
      template: string;
    ...
    results_expired_at?: string;
    upload_expired_at?: string;
  };
};
  ...
  if (!response.ok) {
    ...
    throw new ApiError(
      ...
      typeof body.detail === 'string'
        ? body.detail
        : body.detail?.message || `요청 실패 (${response.status})`,
    );
  }
...
export const resumeUpload = async (
  ...
  while (!signal.aborted) {
    try {
      const state = await uploadRequest<{
        uploaded_bytes: number;
        expected_size: number;
        resumable: boolean;
      }>(`${namespace}/${id}`, { signal });
      if (!state.resumable) {
        if (namespace === '/video-uploads') {
          await uploadRequest(`${namespace}/${id}/complete`, { method: 'POST', signal });
        }
        return;
      }
      if (state.expected_size !== file.size) {
        throw new ApiError(400, '원본 파일과 크기가 일치해야 합니다.');
      }
      let offset = state.uploaded_bytes;
      progress(offset);
      await verifyUploadedPrefix(file, offset, signal, (block) =>
        uploadRequest<void>(`${namespace}/${id}/verify`, {
          method: 'POST',
          signal,
      ...
      while (offset < file.size) {
        ...
        const next = await uploadRequest<{
          uploaded_bytes: number;
        }>(`${namespace}/${id}/chunks?offset=${offset}`, {
          method: 'PUT',
          body: chunk,
          signal,
          headers: { 'Content-Type': 'application/octet-stream' },
        });
        if (next.uploaded_bytes <= offset) {
          throw new Error('업로드 진행 정보를 확인할 수 없습니다.');
        }
      ...
      await new Promise<void>((resolve) => {
        ...
        timer = setTimeout(done, retries * 500);
        signal.addEventListener('abort', done, { once: true });
      });
    }
  }
};
export const uploadRequest = <T>(path: string, init: RequestInit = {}): Promise<T> => {
