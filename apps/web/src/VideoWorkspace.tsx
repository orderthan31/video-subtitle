...
export function VideoWorkspace(){
  ...
  function jobRows(items:Job[]){return <div className="video-job-list">{items.map(job=><article key={job.job_id} className="video-job-row"><div><a className="job-title" href={`#/jobs/${job.job_id}`}>{job.original_filename}</a><p>{templates[job.metadata.workflow?.template||'full']} · {new Date(job.created_at).toLocaleString('ko-KR')}</p><span className={'status '+job.status.toLowerCase()}>{statusLabels[job.status]||job.status}</span>{job.error&&<p className="alert">{job.error}</p>}</div><div className="row-actions">
    ...
    {job.status==='COMPLETED'&&(job.metadata.result_files||[]).map(f=><a className="download" key={f} href={`${base}/jobs/${job.job_id}/results/${f}`}><Download size={14}/>{f==='final.mp4'?'MP4':f==='audio.wav'?'WAV':f}</a>)}
    {job.status==='COMPLETED'&&job.metadata.result_files?.includes('final.mp4')&&<button title="결과 영상을 새 원본으로 등록" disabled={!!busy} onClick={()=>void action(job.job_id,async()=>{const a=await request<VideoAsset>(`/jobs/${job.job_id}/promote-video`,{method:'POST'});location.hash=`/videos/${a.asset_id}`;})}><Plus size={15}/>새 원본 등록</button>}
  ...
  return <div className={'app-shell video-workspace '+(sidebar?'sidebar-open':'sidebar-collapsed')}>
    ...
    {jobId?<JobDetail id={jobId} key={jobId} labels={statusLabels}/>:<main className="video-main">
      ...
      {assetId?<>{detail?<><div className="library-heading"><a className="icon" title="원본 보관함으로" href="#/"><ArrowLeft size={21}/></a><div><p className="eyebrow">원본 영상</p><h1>{detail.video.original_filename}</h1></div><button className="primary" onClick={()=>setWorkflow(true)}><Play size={17}/>새 작업</button></div>
        <section className="asset-overview"><video controls playsInline preload="metadata" src={`${base}/videos/${assetId}/stream`}/><div><h2>영상 정보</h2><dl><dt>길이</dt><dd>{durationLabel(detail.video.media?.duration)||'미확인'}</dd><dt>용량</dt><dd>{sizeLabel(detail.video.size_bytes)}</dd><dt>해상도</dt><dd>{detail.video.media?`${detail.video.media.width} × ${detail.video.media.height}`:'미확인'}</dd><dt>등록일</dt><dd>{new Date(detail.video.created_at).toLocaleString('ko-KR')}</dd><dt>등록 경로</dt><dd>{detail.video.provenance.kind==='result'?'결과 영상에서 등록':'파일 업로드'}{detail.video.provenance.parent_asset_id&&<a href={`#/videos/${detail.video.provenance.parent_asset_id}`}>이전 원본</a>}</dd></dl><button className="danger" onClick={()=>setConfirm({title:'원본 영상을 삭제할까요? 연결된 작업이 있으면 삭제할 수 없습니다.',action:async()=>{await request(`/videos/${assetId}`,{method:'DELETE'});location.hash='/';}})}><Trash2 size={15}/>원본 삭제</button></div></section>
      ...
      </>:<p role="status">영상 불러오는 중</p>}</>:<><div className="library-heading"><div><p className="eyebrow">VIDEO WORKSPACE</p><h1>{jobsView?'전체 작업':'원본 보관함'}</h1></div><button className="primary" onClick={()=>{resume.current=null;input.current?.click();}}><Upload size={17}/>영상 업로드</button></div>
      ...
      {jobsView?jobRows(jobs.filter(j=>j.original_filename.toLocaleLowerCase().includes(query.toLocaleLowerCase()))):<div className="asset-list">{shownVideos.map(v=><a key={v.asset_id} className="asset-row" href={`#/videos/${v.asset_id}`}><div className="asset-thumbnail"><video muted playsInline preload="metadata" src={`${base}/videos/${v.asset_id}/stream#t=0.1`}/></div><div><h2>{v.original_filename}</h2><p>{durationLabel(v.media?.duration)} · {sizeLabel(v.size_bytes)}</p><small>{new Date(v.created_at).toLocaleString('ko-KR')}{v.provenance.kind==='result'?' · 결과에서 등록':''}</small></div><span className="asset-open">열기</span></a>)}{!shownVideos.length&&<div className="library-empty"><FolderOpen size={38}/><p>{query?'검색 결과가 없습니다.':'등록된 원본 영상이 없습니다.'}</p></div>}</div>}
      </>}
    </main>}
