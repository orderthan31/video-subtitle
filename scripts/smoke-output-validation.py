...
def main():
    ...
    encoder = select_encoder(work)
    target = work / "final.mp4"
    video = next(s for s in source_info["streams"] if s["codec_type"] == "video")
    run_process(encoding_args(source, target, video, software=encoder == "libx265"), cwd=work, log_name="encode.log")
    output_info = probe(target, work)
    validate_output(source_info, output_info, target.stat().st_size)
