...
보호 우선 검사는 모든 후보의 재확인이 끝난 뒤 다시 수행한다. 나중에 대사로 재판정된 후보와 앞서 승인된 후보가 겹치거나 0.3초 보호 여유에 들어오면 승인 후보 전체를 철회한다. PCM 처리 계층도 동일 검사를 수행하므로 모순된 삭제/보호 입력에서 보호 구간이 삭제되지 않는다. 후보 순서와 관계없이 적용하고 겹치지 않는 후보는 유지한다.
...
- 후속 충돌 회귀에서 승인 후보가 나중의 대사 판정을 무시하고 PCM을 삭제하는 문제를 먼저 재현했다. 모든 판정 후 보호 우선 및 PCM 계층 재검사를 추가해 해결했다. 후보 순서 반전/분리 후보 유지/모순 입력 PCM 보존을 포함한 비언어 테스트 11개와 전체 Python 177개가 통과했다. 실제 음성 전송 승인 및 모델 품질 검증은 여전히 대기 중이다.
