...
상태/버전 충돌/실행 잠금은 409, 잘못된 트랙·빈 문구·중첩·영상 범위 초과·밀리초보다 짧은 자막은 422다. 트랙당 최대 10,000 cue, cue당 1,000자와 최대 두 줄이다. 저장 시 기존 언어별 줄바꿈·분할 규칙으로 긴 문구를 원래 시간 구간 안에서 나누며 반환된 초안을 새 기준으로 사용한다. 승인 이후에는 편집할 수 없다. 완료된 작업의 원본은 보존하지만 이 API는 검토 대기 상태에서만 편집을 허용한다.

초안 저장 시 기존 파일 외에 새 임시 JSON 전체가 차지할 용량을 검사한다. 서비스 용량/디스크 여유 공간 부족은 507이며 기존 초안과 revision을 유지하고 작업을 승인하지 않는다. 접수 용량 잠금이 사용 중이면 409로 재시도를 요청한다.
...
식별자가 없으면 이전 API처럼 매번 새 작업을 생성한다. 식별자는 작업 메타데이터와 함께 영속화되며, 결과 파일 만료 후에도 작업 이력 보관 기간 동안 유지된다. 사용자가 기록을 삭제하거나 이력 TTL로 정리된 뒤에는 보존되지 않는다. 현재 익명 서비스에서는 전역 식별자이며 인증 기능 추가 시 사용자 범위로 분리해야 한다. 요청 식별자는 인증 수단이 아니다.

## 결과 만료와 작업 이력

`GET /jobs`와 `GET /jobs/{job_id}`는 결과 파일 만료 후에도 이력 보관 기간 동안 작업을 반환한다. 원래 COMPLETED/FAILED/CANCELLED 상태와 완료 시간·옵션·오류를 유지한다. 완료 결과를 정리한 경우 `metadata.results_expired_at`에 정리 시각이 기록되며, 결과 다운로드는 HTTP 410이다. 업로드 만료는 CANCELLED 및 `metadata.upload_expired_at`으로 남긴다.

`DELETE /jobs/{job_id}`는 종료된 작업의 기록과 남은 파일을 함께 삭제한다. 기본 보관 기간은 결과 24시간, 이력은 완료/취소/실패 시각부터 90일이다. Worker의 주기적 정리가 실행될 때 적용한다. 결과 보관 기간보다 짧은 이력 설정은 결과 기간으로 올려 적용한다. 이력 만료 후 조회는 404다.
