...
유효한 작업의 파일 보관은 별도로 `RESULT_TTL_HOURS`(기본 24시간)를 따른다.
완료/실패/취소 직후 원본, 음성, 원문/번역 SRT, 중간 결과, 로그를 삭제하지 않는다.
상시 워커의 반복 실행은 중단 작업의 상태만 복구하며 파일을 정리하지 않는다.

기간 만료 정리는 별도 일회성 배치 `./scripts/run-worker.ps1 -CollectOnly`로 실행한다.
Linux에서는 기존 PYTHONPATH 설정 후 `python -m media_worker.worker --collect-only`를 사용할 수 있다.
운영자가 같은 `VIDEO_STORAGE_ROOT`와 보관 기간을 지정해 cron/작업 스케줄러에서 호출한다.
이 명령은 Gemini 호출이나 작업 처리를 하지 않는다. 이번 변경으로 예약 실행을 생성하거나
현재 파일에 정리 배치를 실행하지 않았다. `.preserve-artifacts` 표식은 수동 삭제와 이 배치를 막지 않는다.
보관 기간이 지난 완료/실패/취소 작업은 input/work/output을 함께 삭제한다.
기존 파일도 배치 실행 시 적용되므로 최초 실행 전에 기간과 저장소를 확인해야 한다.
